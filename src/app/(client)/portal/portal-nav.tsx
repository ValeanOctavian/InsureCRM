...
const navItems = [
  ...
  { label: "Policies", href: "/portal/policies", icon: Shield },
  { label: "Documents", href: "/portal/documents", icon: FileText },
  { label: "Renew", href: "/portal/renew", icon: RefreshCcw },
...
export function PortalNav() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <nav className="fixed left-0 top-0 hidden h-full w-56 flex-col border-r border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 lg:flex">
      ...
      {/* Mobile bottom tabs */}
      <nav className="fixed bottom-0 left-0 right-0 z-40 border-t border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950 md:hidden">
        <div className="flex items-center justify-around px-2 py-1">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={cn("h-5 w-5", isActive(item.href) ? "text-zinc-900 dark:text-zinc-50" : "")} />
                {item.label}
              </Link>
            );
          })}
